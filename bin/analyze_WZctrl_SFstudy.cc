...
#include "hhAnalysis/multilepton/interface/EvtHistManager_WZctrl_SFstudy.h" // EvtHistManager_WZctrl_SFstudy
...
Era era_1;
bool useAssocJetBtag_1 = false;
const Double_t max_jetBtagCSV_1 = get_BtagWP(era_1, Btag::kDeepJet, BtagWP::kMedium);


bool isLeptonPassMVAAndTightSelCuts(const RecoLepton* lepton, const double mvaCut_mu, const double mvaCut_ele) {
  if (lepton->is_muon())
  {
    ...
    if (muon->jetBtagCSV(useAssocJetBtag_1) > max_jetBtagCSV_1)
    {
      return false;
    }
  ...
  } else if (lepton->is_electron())
  {
    ...
    if (electron->jetBtagCSV(useAssocJetBtag_1) > max_jetBtagCSV_1)
    {
      return false;
    }
    if (lepton->mvaRawTTH() > mvaCut_ele) return true;
  }
...
//--- parse command-line arguments
  ...
  std::cout << "<analyze_WZctrl_SFstudy>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("analyze_WZctrl_SFstudy");
...
//--- read python configuration parameters
  ...
  edm::ParameterSet cfg_analyze = cfg.getParameter<edm::ParameterSet>("analyze_WZctrl_SFstudy");
  ...
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here1" << std::endl;
  ...
  const bool   lep_useDifferentMVACutForLepton3 = cfg_analyze.getParameter<bool>("lep_useDifferentMVACutForLepton3");
  const double lep_mva_cut_mu_forLepton3 = cfg_analyze.getParameter<double>("lep_mva_cut_mu_forLepton3");
  const double lep_mva_cut_e_forLepton3  = cfg_analyze.getParameter<double>("lep_mva_cut_e_forLepton3");
  ...
  printf("lep_useDifferentMVACutForLepton3 %d, \t lep_mva_cut_mu_forLepton3 %g, lep_mva_cut_e_forLepton3 %g \n",lep_useDifferentMVACutForLepton3, lep_mva_cut_mu_forLepton3,lep_mva_cut_e_forLepton3);
  ...
  else throw cms::Exception("analyze_WZctrl_SFstudy")
    << "Invalid Configuration parameter 'leptonChargeSelection' = " << leptonChargeSelection_string << " !!\n";

  std::cout << "analyze_WZctrl_SFstudy:: Siddh here2" << std::endl;
  ...
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here3" << std::endl;
  ...
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here4" << std::endl;
  ...
  ;
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here5" << std::endl;
  ...
  switch(era)
  {
    ...
    default: throw cmsException("analyze_WZctrl_SFstudy", __LINE__) << "Invalid era = " << static_cast<int>(era);
  }
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here6" << std::endl;
  ...
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here7  applyFakeRateWeights_string:" << applyFakeRateWeights_string  << ", applyFakeRateWeights: " << applyFakeRateWeights << ", kFR_3lepton: " << kFR_3lepton  << std::endl;
  
  LeptonFakeRateInterface* leptonFakeRateInterface = 0;
  if ( applyFakeRateWeights == kFR_3lepton) {
    edm::ParameterSet cfg_leptonFakeRateWeight = cfg_analyze.getParameter<edm::ParameterSet>("leptonFakeRateWeight");
    std::cout << "analyze_WZctrl_SFstudy:: Siddh here7_1 " << std::endl;
    cfg_leptonFakeRateWeight.addParameter<std::string>("era", era_string);

    std::cout << "analyze_WZctrl_SFstudy:: Siddh here7_2 " << std::endl;

    cfg_leptonFakeRateWeight.addParameter<std::vector<std::string>>("central_or_shifts", central_or_shifts_local);

    leptonFakeRateInterface = new LeptonFakeRateInterface(cfg_leptonFakeRateWeight);
  }
  std::cout << "analyze_WZctrl_SFstudy:: Siddh here8" << std::endl;
...
//--- declare particle collections
  ...
  /*
  //std::string mvaFileName_WZctrl_SFstudy_SUMBk_HH_pkl = "hhAnalysis/multilepton/data/3l_0tau_HH_XGB_noTopness_evtLevelSUM_HH_res_26Var.pkl"; 
  std::string mvaFileName_WZctrl_SFstudy_SUMBk_HH_xml = "hhAnalysis/multilepton/data/3l_0tau_HH_XGB_noTopness_evtLevelSUM_HH_res_26Var.xml";
  std::vector<std::string> mvaInputs_WZctrl_SFstudy_SUMBk_HH = {
  ...
  //XGBInterface mva_xgb_WZctrl_SFstudy_SUMBk_HH(mvaFileName_WZctrl_SFstudy_SUMBk_HH_pkl, mvaInputs_WZctrl_SFstudy_SUMBk_HH);
  TMVAInterface *mva_tmva_WZctrl_SFstudy_SUMBk_HH = new TMVAInterface(mvaFileName_WZctrl_SFstudy_SUMBk_HH_xml, mvaInputs_WZctrl_SFstudy_SUMBk_HH);
  mva_tmva_WZctrl_SFstudy_SUMBk_HH->enableBDTTransform();
  */
...
//--- declare histograms
  struct selHistManagerType
  {
    ...
    std::map<std::string, EvtHistManager_WZctrl_SFstudy*> evt_;
    std::map<std::string, std::map<std::string, EvtHistManager_WZctrl_SFstudy*>> evt_in_categories_;
    std::map<std::string, std::map<std::string, EvtHistManager_WZctrl_SFstudy*>> evt_in_decayModes_;
    std::map<std::string, std::map<std::string, std::map<std::string, EvtHistManager_WZctrl_SFstudy*>>> evt_in_categories_and_decayModes_;
  ...
  for(const std::string & central_or_shift: central_or_shifts_local)
  {
	...
	std::cout << "\t evt_cat_str: " << evt_cat_str << "\t process_string_new: " << process_string_new << ", process_and_genMatchName: " << process_and_genMatchName << std::endl;
        selHistManager->evt_[evt_cat_str] = new EvtHistManager_WZctrl_SFstudy(makeHistManager_cfg(process_and_genMatchName,
	...
	std::cout << "\t category: " << category << std::endl;
        ...
        histogramDir_category.ReplaceAll("WZctrl_3l", Form("%s", category.data()));
	  ...
	  std::cout << "\t\t evt_cat_str: " << evt_cat_str << "\t process_string_new: " << process_string_new << ", process_and_genMatchName: " << process_and_genMatchName << std::endl;
          selHistManager->evt_in_categories_[evt_cat_str][category] = new EvtHistManager_WZctrl_SFstudy(makeHistManager_cfg(process_and_genMatchName,
	  ...
	  std::cout << "\t decayMode_evt: " << decayMode_evt
	    ...
	    std::cout << "\t\t evt_cat_str: " << evt_cat_str << "\t process_string_new: " << process_string_new << ", decayMode_and_genMatchName: " << decayMode_and_genMatchName << std::endl;
            selHistManager -> evt_in_decayModes_[evt_cat_str][decayMode_evt] = new EvtHistManager_WZctrl_SFstudy(makeHistManager_cfg(
            ...
            for(const std::string & category: categories_evt)
            {
              TString histogramDir_category = histogramDir.data();
              histogramDir_category.ReplaceAll("WZctrl_3l", Form("%s", category.data()));
              selHistManager -> evt_in_categories_and_decayModes_[evt_cat_str][category][decayMode_evt] = new EvtHistManager_WZctrl_SFstudy(makeHistManager_cfg(
                decayMode_and_genMatchName,
  ...
  const std::vector<std::string> cuts = {
    ...
    "signal region veto; woLeptonSF event weight",
    "signal region veto; 2lSFOS_plus_mu; woLeptonSF event weight",
    "signal region veto; 2lSFOS_plus_e; woLeptonSF event weight"
 };
...
//--- build collections of generator level particles (before any cuts are applied, to check distributions in unbiased event samples)
    ...
    std::cout << "isMC: " << isMC << ", fillGenEvtHistograms: "<< fillGenEvtHistograms << std::endl;
...
//--- tight selection for WZ control region
    std::vector<const RecoLepton*> tightLeptonsFull;
    if ( ! lep_useDifferentMVACutForLepton3 )
    {    
      // default way
      tightLeptonsFull = mergeLeptonCollections(tightElectrons, tightMuons, isHigherConePt);
    } else
    {	
      // 2 SFOS leptons with mva > lep_mva_cut; 3rd lepton with mva > lep_mva_cut_<>_forLepton3
      for ( std::vector<const RecoLepton*>::const_iterator lepton1 = fakeableLeptonsFull.begin();
	    lepton1 != fakeableLeptonsFull.end(); ++lepton1 ) {
	for ( std::vector<const RecoLepton*>::const_iterator lepton2 = lepton1 + 1;
	      lepton2 != fakeableLeptonsFull.end(); ++lepton2 ) {
	  if ( (*lepton1)->pdgId() == -(*lepton2)->pdgId() ) { // pair of same flavor leptons of opposite charge
	    double mass = ((*lepton1)->p4() + (*lepton2)->p4()).mass();
	    if ( std::fabs(mass - z_mass) < z_window ) {
	      if ((isLeptonPassMVAAndTightSelCuts(*lepton1, lep_mva_cut_mu, lep_mva_cut_e)) &&
		  (isLeptonPassMVAAndTightSelCuts(*lepton2, lep_mva_cut_mu, lep_mva_cut_e)) )
	      {
		tightLeptonsFull.push_back(*lepton1);
		tightLeptonsFull.push_back(*lepton2);
	      }	      
	    }	    
	  }
	}
      }
      // check remaining leptons from fakeableLeptonsFull
      for ( std::vector<const RecoLepton*>::const_iterator lepton1 = fakeableLeptonsFull.begin();
	    lepton1 != fakeableLeptonsFull.end(); ++lepton1 ) {
	bool goto_next = false;
	for ( std::vector<const RecoLepton*>::const_iterator lepton2 = tightLeptonsFull.begin();
	      lepton2 != tightLeptonsFull.end(); ++lepton2 ) {
	  if ((*lepton1) == (*lepton2)) {
	    goto_next = true;
	    break;
	  }
	...
	if (isLeptonPassMVAAndTightSelCuts(*lepton1, lep_mva_cut_mu_forLepton3, lep_mva_cut_e_forLepton3)) tightLeptonsFull.push_back(*lepton1);
      }
      std::sort(tightLeptonsFull.begin(), tightLeptonsFull.end(), isHigherConePt);
    }
    
    if(isDEBUG || run_lumi_eventSelector)
    {
      printCollection("fakeableLeptonsFull", fakeableLeptonsFull);
...
//    to also pass the tight identification and isolation criteria
	...
	if (useLeptonSF_LooseToTight) { // <<<<<<<<<<<<<<<<<<<<<<<< TEMPORARY for check
...
//--- retrieve gen-matching flags    
    ...
    if (genMatches.size() != 1) {
      std::cout << "analyze_WZctrl_SFstudy: GenMatches in an event: " << genMatches.size() << "\t\t\t ERROR: Code is not ready for it ****" << std::endl;
      throw cmsException("analyze_WZctrl_SFstudy", __LINE__) << " GenMatches in an event: " << genMatches.size() << "\t\t\t ERROR: Code is not ready for it **** \n";
    }
    ...
    if ( isDEBUG || run_lumi_eventSelector ) {
      ...
      std::cout << "\nevtWeightRecorde_woLeptonSF: " << evtWeightRecorder_woLeptonSF << "\n";
      std::cout << "\nevtWeightRecorde_woLeptonSFCor: " << evtWeightRecorder_woLeptonSFCor << "\n";
    ...
    int    lepton3PID  = selLepton_lepton3 ? selLepton_lepton3->pdgId() : -1;
    double lepton3_pt  = selLepton_lepton3 ? selLepton_lepton3->pt() : -1;
    double lepton3_eta = selLepton_lepton3 ? selLepton_lepton3->eta() : -10;
    
    std::vector<std::string> evtCategories;
    evtCategories.push_back("WZctrl_3l_woLeptonSF");
    evtCategories.push_back("WZctrl_3l_woLeptonSFCor");
    if      (nEle_3selLep == 3 && nMu_3selLep == 0) evtCategories.push_back("WZctrl_3e");
    else if (nEle_3selLep == 2 && nMu_3selLep == 1) evtCategories.push_back("WZctrl_2e1mu");
    else if (nEle_3selLep == 1 && nMu_3selLep == 2) evtCategories.push_back("WZctrl_1e2mu");
    else if (nEle_3selLep == 0 && nMu_3selLep == 3) evtCategories.push_back("WZctrl_3mu");
    else {
    ...
    else if (nEle_3selLep == 1 && nMu_3selLep == 2) evtCategories.push_back("WZctrl_1e2mu_woLeptonSFCor");
    else if (nEle_3selLep == 0 && nMu_3selLep == 3) evtCategories.push_back("WZctrl_3mu_woLeptonSFCor");
    else {
      printf("Invalid event category\t\t nEle_3selLep %d, nMu_3selLep %d",nEle_3selLep,nMu_3selLep);
    }
    if      (selLepton_lepton3 && selLepton_lepton3->is_muon())     evtCategories.push_back("WZctrl_2lSFOS_plus_mu_woLeptonSFCor");
    else if (selLepton_lepton3 && selLepton_lepton3->is_electron()) evtCategories.push_back("WZctrl_2lSFOS_plus_e_woLeptonSFCor");
    
    //sTmp123 += Form(" isVBF: %i, ",(int)isVBF);

    if ( isDEBUG || run_lumi_eventSelector || printLevel > 5) {
    ...
    if (selLepton_lepton3 && selLepton_lepton3->is_electron()) 
      cutFlowTable.update("signal region veto; 2lSFOS_plus_e", evtWeightRecorder.get(central_or_shift_main));


    cutFlowTable.update("signal region veto; woLeptonSF event weight", evtWeightRecorder_woLeptonSF.get(central_or_shift_main));
    if (selLepton_lepton3 && selLepton_lepton3->is_muon())
      cutFlowTable.update("signal region veto; 2lSFOS_plus_mu; woLeptonSF event weight", evtWeightRecorder_woLeptonSF.get(central_or_shift_main));
    if (selLepton_lepton3 && selLepton_lepton3->is_electron())
    ...
    if (selLepton_lepton3 && selLepton_lepton3->is_electron())
      cutFlowTable.update("signal region veto; 2lSFOS_plus_e; woLeptonSFCor event weight", evtWeightRecorder_woLeptonSFCor.get(central_or_shift_main));

    if ( ! selLepton_lepton3)
      cutFlowTable.update("signal region veto; selLepton_lepton3=0", evtWeightRecorder.get(central_or_shift_main));
	...
	if (mT < 0.) std::cout << "analyze_WZctrl_SFstudy:: mT (MET+Lepton) < 0 \t\t *** ERROR ***" << std::endl;
    ...
    */
	...
	dr_los_max = dr1;
      ...
      std::cout << "analyze_WZctrl_SFstudy: Error in calculating dr_os " << std::endl;
      throw cmsException("analyze_WZctrl_SFstudy", __LINE__) << "Error in calculating dr_os \n";
    }
    ...
    */
	...
	(selLepton2_H_WW_ll_Approach2->p4() + selLepton_H_WW_ljj_Approach2->p4()).mass() ) { // mistake in picking leptons in Approach 2
      std::cout << "analyze_WZctrl_SFstudy: mistake in picking leptons in Approach 2" << std::endl;
      throw cmsException("analyze_WZctrl_SFstudy", __LINE__) << "Error in calculating dr_os n";
    }
    ...
    */
    ...
    /*
    const std::map<std::string, double>  mvaInputVariables_WZctrl_SFstudy_SUMBk_HH = {
    ...
    //const double mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH_1 = 100; //mva_xgb_WZctrl_SFstudy_SUMBk_HH(mvaInputVariables_WZctrl_SFstudy_SUMBk_HH);
    const double mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH = 0.0; //(*mva_tmva_WZctrl_SFstudy_SUMBk_HH)(mvaInputVariables_WZctrl_SFstudy_SUMBk_HH);
    const double mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH = 0.0; //mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH; // #### IMPORTANT ************
    
    /*
    printf("mvaInputVariables_WZctrl_SFstudy_SUMBk_HH:: \n");
    for (std::map<std::string, double>::const_iterator it=mvaInputVariables_WZctrl_SFstudy_SUMBk_HH.begin(); it != mvaInputVariables_WZctrl_SFstudy_SUMBk_HH.end(); it++) {
      std::cout << "\t" << it->first << " \t\t  " << it->second << std::endl;      
    }
    std::cout << "mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH_1 " << mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH_1 << std::endl;
    std::cout << "mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH " << mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH << std::endl;
    std::cout << "mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH " << mvaOutput_xgb_WZctrl_SFstudy_SUMBk_HH << std::endl;
    */
...
//--- fill histograms with events passing final selection
    ...
    */
    ...
    std::string stmp1 = Form("isWjjHasOnly1j %d, numSameFlavor_OS %d, mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH %g \n",isWjjHasOnly1j, numSameFlavor_OS, mvaOutput_tmva_WZctrl_SFstudy_SUMBk_HH);
    if (1==0) {     
      std::cout << stmp1;
      std::cout << selLepton_H_WW_ljj_Approach0 << selLepton1_H_WW_ll_Approach0 << selLepton2_H_WW_ll_Approach0;
    }
	...
	if (printLevel > 5)
	{
	  std::cout << "genMatch Idx: " << genMatch->getIdx() << ", name: " << genMatch->getName() << "\n";
	}
	...
	for(const auto & kv: reWeightMapHH)
	  ...
	  if (printLevel > 5)
	  {
	    ...
	    if (printLevel > 5)
	    {
	      std::cout << "\t\t decayModeStr: " <<  decayModeStr << "\n";
	    }
	  ...
	  for(const auto & kv: reWeightMapHH_toUse)
	    ...
	    if (printLevel > 5)
	    {
	      std::cout << "\t\t kv: " << kv.first << ", " << kv.second << "\n";
            ...
            EvtHistManager_WZctrl_SFstudy* selHistManager_evt_category = selHistManager->evt_in_categories_[kv.first][category];
		...
		std::cout << "\t\t\t decayModeStr: " <<  decayModeStr << "\n";
              ...
              if (! decayModeStr.empty()) {
                ...
                EvtHistManager_WZctrl_SFstudy* selHistManager_evt_category_decMode = selHistManager->evt_in_categories_and_decayModes_[kv.first][category][decayModeStr];
...
//--- memory clean-up
  ...
  clock.Show("analyze_WZctrl_SFstudy");
