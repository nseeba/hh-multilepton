...
void dumpGenParticles(const std::string &label,
                      const std::vector<GenParticle> &particles) {
...
void printWjj(const std::vector<const RecoJetAK8 *> &jets_ak8,
  ...
  for (size_t idxWBoson = 0; idxWBoson < genWBosons.size(); ++idxWBoson) {
    ...
    std::cout << " genWBoson #" << idxWBoson << ": pT = " << genWBoson.pt()
              << ", eta = " << genWBoson.eta() << ", phi = " << genWBoson.phi()
              << std::endl;
  }
  ...
  for (size_t idxWJet = 0; idxWJet < genWJets.size(); ++idxWJet) {
    ...
    std::cout << " genWJet #" << idxWJet << ": pT = " << genWJet.pt()
              << ", eta = " << genWJet.eta() << ", phi = " << genWJet.phi()
              << std::endl;
  }
  if (genWBosons.size() == 1) {
    bool isMatched = false;
    Particle::LorentzVector genWjjP4 = genWBosons[0].p4();
    std::cout << "genWjj: pT = " << genWjjP4.pt()
              << ", eta = " << genWjjP4.eta() << ", phi = " << genWjjP4.phi()
              << std::endl;
    for (std::vector<const RecoJetAK8 *>::const_iterator jet_ak8 =
             jets_ak8.begin();
         jet_ak8 != jets_ak8.end(); ++jet_ak8) {
      double dR = deltaR(genWjjP4, (*jet_ak8)->p4());
      if (dR < 0.8) {
        std::cout << "matches reconstructed AK8 jet: pT = " << (*jet_ak8)->pt()
                  << ", eta = " << (*jet_ak8)->eta()
                  << ", phi = " << (*jet_ak8)->phi() << ","
                  << " msoftdrop = " << (*jet_ak8)->msoftdrop()
                  << ", tau21 = " << (*jet_ak8)->tau2() / (*jet_ak8)->tau1()
                  << ", which ";
        ...
        for (std::vector<GenParticle>::const_iterator genWJet1 =
                 genWJets.begin();
          ...
          for (std::vector<GenParticle>::const_iterator genWJet2 = genWJet1 + 1;
            ...
            if (deltaR(genWJet1->p4() + genWJet2->p4(), genWjjP4) < 1.e-1 &&
                std::fabs((genWJet1->p4() + genWJet2->p4()).mass() -
                          genWjjP4.mass()) < 1.e+1) {
              std::cout << " genWJet #1: pT = " << genWJet1->pt()
                        << ", eta = " << genWJet1->eta()
                        << ", phi = " << genWJet1->phi() << std::endl;
              std::cout << " genWJet #2: pT = " << genWJet2->pt()
                        << ", eta = " << genWJet2->eta()
                        << ", phi = " << genWJet2->phi() << std::endl;
            }
          }
        }
        ...
        if (subjet1)
          std::cout << " subjet #1: pT = " << subjet1->pt()
                    << ", eta = " << subjet1->eta()
                    << ", phi = " << subjet1->phi() << std::endl;
        const RecoSubjetAK8 *subjet2 = (*jet_ak8)->subJet2();
        if (subjet2)
          std::cout << " subjet #2: pT = " << subjet2->pt()
                    << ", eta = " << subjet2->eta()
                    << ", phi = " << subjet2->phi() << std::endl;
      }
    }
    if (genWjjP4.pt() > 100. && !isMatched)
      std::cout << "--> DEBUG (Wjj) !!" << std::endl;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // process_string.find("_hh_") != std::string::npos;
  ...
  if (isDEBUG)
    std::cout << "Warning: DEBUG mode enabled -> trigger selection will not be "
                 "applied for data !!"
              << std::endl;
  ...
  // cfg_analyze.getParameter<std::string>("branchName_jets"); std::string
  // branchName_jets_ak8 =
  // cfg_analyze.getParameter<std::string>("branchName_jets_ak8"); std::string
  // branchName_subjets_ak8 =
  // cfg_analyze.getParameter<std::string>("branchName_subjets_ak8");
  std::string branchName_jets =
  ...
  /*
  const edm::ParameterSet mvaInfo_nonRes_base =
  cfg_analyze.getParameter<edm::ParameterSet>("mvaInfo_nonRes_base");
  std::string BDTFileName_nonRes_base_even  =
  mvaInfo_nonRes_base.getParameter<std::string>("BDT_xml_FileName_nonRes_base_even");
  std::string BDTFileName_nonRes_base_odd   =
  mvaInfo_nonRes_base.getParameter<std::string>("BDT_xml_FileName_nonRes_base_odd");
  std::vector<std::string> BDTInputVariables_nonRes_base =
  mvaInfo_nonRes_base.getParameter<std::vector<std::string>>("inputVars_nonRes_base");
  // Include all Input Var.s except BM indices
  */
  ...
  TMVAInterface *BDT_spin0 =
      new TMVAInterface(BDTFileName_spin0_odd, BDTFileName_spin0_even,
                        BDTInputVariables_spin0, fitFunctionFileName_spin0);
  ...
  TMVAInterface *BDT_spin2 =
      new TMVAInterface(BDTFileName_spin2_odd, BDTFileName_spin2_even,
                        BDTInputVariables_spin2, fitFunctionFileName_spin2);
  ...
  TMVAInterface *BDT_nonRes =
      new TMVAInterface(BDTFileName_nonRes_odd, BDTFileName_nonRes_even,
                        BDTInputVariables_nonRes);
  ...
  TMVAInterface* BDT_nonRes_base = new
  TMVAInterface(BDTFileName_nonRes_base_odd, BDTFileName_nonRes_base_even,
  BDTInputVariables_nonRes_base); BDT_nonRes_base->enableBDTTransform();
  ...
  */
  ...
  "hhAnalysis/multilepton/data/hh_2lss_XGB_noTopness_evtLevelSUM_HH_res_10Var.pkl";
  ...
  */
  ...
  for (const std::string &central_or_shift : central_or_shifts_local) {
    ...
    for (const GenMatchEntry *genMatchDefinition : genMatchDefinitions) {
      ...
      if (!skipBooking) {
        selHistManager->evt_ = new EvtHistManager_hh_2lss(makeHistManager_cfg(
      ...
      if (!skipBooking) {
        // CV: make correlation plot for MVA input variables used for
        // non-resonant HH signal extraction,
        //     as defined in
        //     hhAnalysis/multilepton/data/BDT_2lss/nonres_default_Sandeep_9vars_usingLepConePt/2lss_even_half_model_nonres_default.xml
        std::vector<std::string> mvaInputVariables = {
            "mht",     "HT",          "lep1_conePt",    "mindr_lep1_jet",
            "mT_lep1", "lep2_conePt", "mindr_lep2_jet", "mT_lep2",
            "dR_ll",   "max_lep_eta"};
        ...
        // selHistManager->mvaInputVarCorrelation_ee_ = new
        // MVAInputVarCorrelationHistManager(makeHistManager_cfg(process_and_genMatch,
        //    Form("%s/sel/mvaInputVarCorrelation_ee", histogramDir.data()),
        //    era_string, central_or_shift));
        // selHistManager->mvaInputVarCorrelation_ee_->bookHistograms(fs,
        // mvaInputVariables); selHistManager->mvaInputVarCorrelation_em_ = new
        // MVAInputVarCorrelationHistManager(makeHistManager_cfg(process_and_genMatch,
        //    Form("%s/sel/mvaInputVarCorrelation_em", histogramDir.data()),
        //    era_string, central_or_shift));
        // selHistManager->mvaInputVarCorrelation_em_->bookHistograms(fs,
        // mvaInputVariables); selHistManager->mvaInputVarCorrelation_mm_ = new
        // MVAInputVarCorrelationHistManager(makeHistManager_cfg(process_and_genMatch,
        //    Form("%s/sel/mvaInputVarCorrelation_mm", histogramDir.data()),
        //    era_string, central_or_shift));
        // selHistManager->mvaInputVarCorrelation_mm_->bookHistograms(fs,
        // mvaInputVariables);
      }
  ...
  if (selectBDT) {
    ...
    bdt_filler->register_variable<float_type>(
        "dihiggsVisMass_sel", "dihiggsMass_wMet_sel", "jetMass_sel",
        "leptonPairMass_sel", "leptonPairCharge_sel", "met", "mht", "met_LD",
        "HT", "STMET", "evtWeight", "lep1_pt", "lep1_conePt", "lep1_eta",
        "mindr_lep1_jet", "mT_lep1", "lep2_pt", "lep2_conePt", "lep2_eta",
        "mindr_lep2_jet", "mT_lep2", "dR_ll", "pT_ll", "max_lep_eta",
        "pT_llMEt", "Smin_llMEt", "vbf_m_jj", "vbf_dEta_jj", "THWeight",
        "mhh_gen", "costS_gen", "genWeight", "lheWeight", "pileupWeight",
        "triggerWeight", "btagWeight", "leptonEffSF", "data_to_MC_correction",
        "FR_Weight", "lep1_frWeight", "lep2_frWeight", "evtWeight");
  ...
  while (inputTree->hasNextEvent() &&
    ...
    const std::vector<const RecoJet *> cleanedJets =
        jetCleaningByIndex
            ...
            : jetCleaner(jet_ptrs,
                         ...
                         fakeableHadTaus)
        // jetCleanerByIndex(jet_ptrs, selectBDT ? selLeptons : fakeableLeptons,
        // std::vector<const RecoHadTau*>({})) : jetCleaner       (jet_ptrs,
        // selectBDT ? selLeptons : fakeableLeptons, std::vector<const
        // RecoHadTau*>({}))
        ;
    ...
    if ((int)selHadTaus.size() > maxNumTaus) {
      if (run_lumi_eventSelector) {
        std::cout << "event " << eventInfo.str() << " FAILS selHadTaus veto."
                  << std::endl;
        printCollection("selHadTaus", selHadTaus);
      }
      continue;
    ...
    cutFlowTable.update("sel tau veto",
                        evtWeightRecorder.get(central_or_shift_main));
    cutFlowHistManager->fillHistograms(
        "sel tau veto", evtWeightRecorder.get(central_or_shift_main));
    ...
    selJets.begin(); selJet1_W != selJets.end(); ++selJet1_W ) { for (
    std::vector<const RecoJet*>::const_iterator selJet2_W = selJet1_W + 1;
    ...
    mass_jj_W==0) { mass_jj_W = mass_jj; selJetP4 = (*selJet1_W)->p4() +
    (*selJet2_W)->p4(); int counter = 0; for ( std::vector<const
    RecoJet*>::const_iterator selJet3_4 = selJets.begin(); selJet3_4 !=
    selJets.end(); ++selJet3_4 ) { if ((*selJet3_4)!=(*selJet1_W) &&
    (*selJet3_4)!=(*selJet2_W)){ counter++; if (counter <= 2){ selJetP4 +=
    (*selJet3_4)->p4();
              }
            }
          }
    ...
    if (selJetsAK8_Wjj_selected.size() >= 2) { // boosted category
      ...
      if (idxWJet1 != 9999 && idxWJet2 != 9999) { // for 2 AK4 jets case
        ...
        selJetP4 =
            (selJetsAK8_Wjj_selected[0]->subJet1()->p4() +
             selJetsAK8_Wjj_selected[0]->subJet2()->p4() + selJets[0]->p4());
        isEventSemiboosted = true;
      }
      ...
      for (std::vector<const RecoJet *>::const_iterator selJet1_W =
        ...
        for (std::vector<const RecoJet *>::const_iterator selJet2_W =
          ...
          if (std::fabs(mass_jj - w_mass) < std::fabs(mass_jj_W - w_mass) ||
            ...
            int counter = 0;
            for (std::vector<const RecoJet *>::const_iterator selJet3_4 =
                     selJets.begin();
                 selJet3_4 != selJets.end(); ++selJet3_4) {
              if ((*selJet3_4) != (*selJet1_W) &&
                  (*selJet3_4) != (*selJet2_W)) {
                counter++;
                if (counter <= 2) {
    ...
    const bool failsZbosonMassVeto =
        ...
        (selLepton_lead->is_electron() && selLepton_sublead->is_electron() &&
    ...
    cutFlowTable.update("Z-boson mass veto",
                        evtWeightRecorder.get(central_or_shift_main));
    cutFlowHistManager->fillHistograms(
        "Z-boson mass veto", evtWeightRecorder.get(central_or_shift_main));
    ...
    double leptonPairMass_sel =
        (selLepton_lead->cone_p4() + selLepton_sublead->cone_p4()).mass();
    double leptonPairCharge_sel =
    ...
    BDTInputs_SUM["mT_lep1"]              = comp_MT_met(selLepton_lead,
    met.pt(), met.phi()); BDTInputs_SUM["mindr_lep2_jet"]       = std::min(10.,
    mindr_lep2_jet); BDTInputs_SUM["mT_lep2"]              =
    comp_MT_met(selLepton_sublead, met.pt(), met.phi()); BDTInputs_SUM["dR_ll"]
    = dR_ll; BDTInputs_SUM["max_lep_eta"]          =
    TMath::Max(std::abs(selLepton_lead -> eta()), std::abs(selLepton_sublead ->
    eta()));
    ...
    */
    ...
    for (std::vector<const RecoJet *>::const_iterator selJetVBF1 =
      ...
      for (std::vector<const RecoJet *>::const_iterator selJetVBF2 =
        ...
        double dEta_jj =
            TMath::Abs((*selJetVBF1)->eta() - (*selJetVBF2)->eta());
    ...
    AllVars_Map["gen_mHH"] = 250.; // setting a Dummy value which will be reset
                                   // depending on mass hypothesis
    ...
    AllVars_Map["lep1_conePt"] =
        std::min(120., comp_lep_conePt(*selLepton_lead));
    AllVars_Map["lep1_eta"] = selLepton_lead->eta();
    AllVars_Map["mindr_lep1_jet"] = std::min(10., mindr_lep1_jet);
    AllVars_Map["mT_lep1"] =
        std::min(150., comp_MT_met(selLepton_lead, met.pt(), met.phi()));
    AllVars_Map["lep2_pt"] = std::min(120., selLepton_sublead->pt());
    AllVars_Map["lep2_conePt"] =
        std::min(120., comp_lep_conePt(*selLepton_sublead));
    AllVars_Map["lep2_eta"] = selLepton_sublead->eta();
    AllVars_Map["mindr_lep2_jet"] = std::min(10., mindr_lep2_jet);
    AllVars_Map["mT_lep2"] =
        std::min(150., comp_MT_met(selLepton_sublead, met.pt(), met.phi()));
    AllVars_Map["dR_ll"] = dR_ll;
    AllVars_Map["pT_ll"] = pT_ll;
    AllVars_Map["max_lep_eta"] = TMath::Max(std::abs(selLepton_lead->eta()),
    ...
    BDTOutput_Map_spin2 = CreateResonantBDTOutputMap(
        gen_mHH, BDT_spin2, BDTInputs_spin2, eventInfo.event, "_spin2");
    BDTOutput_Map_spin0 = CreateResonantBDTOutputMap(
        gen_mHH, BDT_spin0, BDTInputs_spin0, eventInfo.event, "_spin0");
    ...
    // BDTOutput_Map_nonRes_base = CreateBDTOutputMap(nonResBase_params,
    // BDT_nonRes_base, BDTInputs_nonRes_base, eventInfo.event, true, "_base");
      ...
      for (const GenMatchEntry *genMatch : genMatches) {
        ...
        if (!skipFilling) {
          selHistManager->evt_->fillHistograms(
              ...
              jetMass_sel, leptonPairMass_sel,
              (selLepton_lead->is_electron() &&
               selLepton_sublead->is_electron())
                  ? leptonPairMass_sel
                  : -1.,
              (selLepton_lead->is_muon() && selLepton_sublead->is_muon())
                  ? leptonPairMass_sel
                  : -1.,
              leptonPairCharge_sel, metP4.pt(), mhtP4.pt(), met_LD, HT, STMET,
              //
              lep1_conePt, std::min(10., mindr_lep1_jet), mT_lep1,
              deltaPhi(selLepton_lead->phi(), metP4.phi()),
              deltaPhi(selLepton_lead->phi(), mhtP4.phi()),
              //
              lep2_conePt, std::min(10., mindr_lep2_jet), mT_lep2,
        ...
        selHistManager->datacard_->fillHistograms(
            BDTOutput_Map_spin2, BDTOutput_Map_spin0, BDTOutput_Map_nonRes,
        ...
        if (!skipFilling) {
          // static std::map<selHistManagerType*, bool>
          // mvaInputVarCorrelation_isBooked; // key = selHistManager if (
          // !mvaInputVarCorrelation_isBooked[selHistManager] )
          //{
          ...
          // selLepton_sublead->is_electron() )
          //{
          //  selHistManager->mvaInputVarCorrelation_ee_->fillHistograms(AllVars_Map,
          //  evtWeight);
          //}
          // if ( (selLepton_lead->is_electron() &&
          // selLepton_sublead->is_muon()) || (selLepton_lead->is_muon() &&
          // selLepton_sublead->is_electron()) )
          //{
          //  selHistManager->mvaInputVarCorrelation_em_->fillHistograms(AllVars_Map,
          //  evtWeight);
          //}
          // if ( selLepton_lead->is_muon() && selLepton_sublead->is_muon() )
          //{
          //  selHistManager->mvaInputVarCorrelation_mm_->fillHistograms(AllVars_Map,
          //  evtWeight);
          //}
        }
    ...
    if (bdt_filler) {
      ...
      bdt_filler
          ...
          operator()({eventInfo.run, eventInfo.lumi, eventInfo.event})(
              ...
              "jetMass_sel", jetMass_sel)("leptonPairMass_sel",
                                          leptonPairMass_sel)(
              "leptonPairCharge_sel", leptonPairCharge_sel)("met", metP4.pt())(
              "mht", mhtP4.pt())("met_LD", met_LD)("HT", HT)("STMET", STMET)(
              "evtWeight", evtWeightRecorder.get(central_or_shift_main))(
              "lep1_pt", selLepton_lead->pt())(
              "lep1_conePt", comp_lep_conePt(*selLepton_lead))(
              "lep1_eta", selLepton_lead->eta())("mindr_lep1_jet",
              ...
              "dR_ll", dR_ll)("pT_ll", pT_ll)(
              "max_lep_eta", TMath::Max(std::abs(selLepton_lead->eta()),
                                        std::abs(selLepton_sublead->eta())))(
              "pT_llMEt", pT_llMEt)("Smin_llMEt", Smin_llMEt)("vbf_dEta_jj",
                                                              vbf_dEta_jj)(
              "vbf_m_jj", vbf_m_jj)("nJet", comp_n_jet25_recl(selJets))(
              "nJet_vbf", selJetsVBF.size())("isVBF", isVBF)(
              "nLep", selLeptons.size())("nLep_loose",
                                         preselLeptonsFull.size())(
              "nElec", selElectrons.size())("THWeight", THWeight)("BM", BM)(
              "mhh_gen", mhh_gen)("costS_gen", costS_gen)("lep1_frWeight",
                                                          lep1_frWeight)(
              "lep2_frWeight", lep2_frWeight)("genWeight", eventInfo.genWeight)(
              "lheWeight",
              evtWeightRecorder.get_lheScaleWeight(central_or_shift_main))(
              "pileupWeight",
              evtWeightRecorder.get_puWeight(central_or_shift_main))(
              "triggerWeight",
              evtWeightRecorder.get_sf_triggerEff(central_or_shift_main))(
              "btagWeight", evtWeightRecorder.get_btag(central_or_shift_main))(
              "leptonEffSF", evtWeightRecorder.get_leptonSF())(
              "data_to_MC_correction",
              evtWeightRecorder.get_data_to_MC_correction(
                  central_or_shift_main))(
              "FR_Weight", evtWeightRecorder.get_FR(central_or_shift_main))(
              "evtWeight", evtWeight_BDT)(weightMapHH)
          .fill();
    }
