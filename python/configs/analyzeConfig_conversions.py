...
class analyzeConfig_conversions(analyzeConfig_hh):
  """Configuration metadata needed to run analysis in a single go.

  Sets up a folder structure by defining full path names; no directory creation is delegated here.

  Args specific to analyzeConfig_conversions:
  ...
  def __init__(self,
    ...
    '''
    print "analyzeConfig_conversions::__init__: lepton_genMatches_nonfakes:",self.lepton_genMatches_nonfakes,\
    ...
    self.histogramDir_prep_dcard = "conversions_OS_Tight"
    self.histogramDir_prep_dcard_SS = "conversions_SS_Tight"
    self.make_plots_backgrounds = self.get_makeplots_backgrounds()

    self.cfgFile_make_plots = os.path.join(self.template_dir, "makePlots_conversions_cfg.py")
    self.cfgFile_make_plots_mcClosure = os.path.join(self.template_dir, "makePlots_mcClosure_conversions_cfg.py") #TODO
    ...
    self.category_inclusive = "conversions"
  ...
  def createCfg_analyze(self, jobOptions, sample_info, lepton_selection):
    """Create python configuration file for the analyze_conversions executable (analysis code)
    ...
    Args:
      ...
      central_or_shift: either 'central' or one of the systematic uncertainties defined in $CMSSW_BASE/src/hhAnalysis/multilepton/bin/analyze_conversions.cc
    """
    ...
    lines = super(analyzeConfig_conversions, self).createCfg_analyze(jobOptions, sample_info)
    ...
    for category in self.categories:
      for histogramToFit in self.histograms_to_fit:
        ...
        for leptonChargeSelection in self.leptonChargeSelections:          
          ...
          histogramDir_nominal = "%s/sel/evt/fakes_mc" % getHistogramDir(category, "Tight", "disabled", leptonChargeSelection)  
          for lepton_type in [ 'e', 'm' ]:
            ...
            histogramDir_mcClosure = "%s/sel/evt/fakes_mc" % self.mcClosure_dir['%s_%s' % (lepton_mcClosure, leptonChargeSelection)]
            if "BDTOutput" in histogramToFit or "MVAOutput" in histogramToFit:
